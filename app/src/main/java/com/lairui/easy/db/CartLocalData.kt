...
class CartLocalData private constructor() {
    ...
    fun selectDB(): List<MutableMap<String, Any>> {
        ...
        openDb()
        if (db!!.isOpen) {
            val cursor = db!!.rawQuery("select * from tb_goods_cart where is_work = 0", null)
            val idIndex = cursor.getColumnIndex("id")
            val goodsIdIndex = cursor.getColumnIndex("goods_id")
            val goodsCountIndex = cursor.getColumnIndex("goods_count")
            val goodsAmountIndex = cursor.getColumnIndex("goods_price")
    ...
    fun insertDB(goodsId: Int, goodsCount: Int, goodsAmount: String) {
        openDb()
        if (db!!.isOpen) {
    ...
    companion object {
        ...
        private var sqliteDBHelp: CartLocalData? = null
        val instance: CartLocalData
            get() {
                if (sqliteDBHelp == null) {
                    sqliteDBHelp = CartLocalData()
                }
                return sqliteDBHelp as CartLocalData
            }
    }
