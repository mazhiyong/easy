...
class TestScanActivity : BasicActivity(), QRCodeView.Delegate {
    ...
    lateinit var mLightView: ImageView
    @BindView(R.id.select_image_tv)
    lateinit var mSelectImageTv: TextView
    @BindView(R.id.zxingview)
    lateinit var mZXingView: ZXingView
    @BindView(R.id.top_layout)
    lateinit var mTopLayout: LinearLayout
    @BindView(R.id.flashLightIv)
    lateinit var mFlashLightIv: AppCompatImageView
    @BindView(R.id.flashLightTv)
    lateinit var mFlashLightTv: TextView
    @BindView(R.id.flashLightLayout)
    lateinit var mFlashLightLayout: LinearLayoutCompat
    @BindView(R.id.albumIv)
    lateinit var mAlbumIv: AppCompatImageView
    @BindView(R.id.albumLayout)
    lateinit var mAlbumLayout: LinearLayoutCompat
    ...
    override val contentView: Int
        get() = R.layout.activity_test_scan
    ...
    override fun init() {
        ...
        val layoutParams = RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, this.resources.getDimension(R.dimen.title_item_height).toInt() + UtilTools.getStatusHeight2(this))
    ...
    override fun onScanQRCodeSuccess(result: String) {
        ...
        when (mType) {
            ...
            -> {
                //将发票信息录入数据库
                val fp_code = "100120123"
                val fp_number = "100120111301"
                val fp_money = "100.00"
                val fp_date = "2019-04-19"

                if (FaPiaoData.instance.dataExist(fp_code, fp_number)) {
                    Toast.makeText(this@TestScanActivity, "当前发票信息已存在,请换一张试试", Toast.LENGTH_LONG).show()
                } else {
                    FaPiaoData.instance.insertDB(fp_code, fp_number, fp_money, fp_date)
                }
                intent = Intent(this, InvoiceListActivity::class.java)
                startActivity(intent)
            }
            "3" //注册
            ->
                ...
                if (result.contains("i=")) {
                    val i = result.substring(result.indexOf("i=") + 2)
                    intent = Intent(this@TestScanActivity, RegistActivity::class.java)
    ...
    override fun onCameraAmbientBrightnessChanged(isDark: Boolean) {
        ...
        if (isDark) {
            ...
            mLightView!!.visibility = View.VISIBLE
        } else {
            if (tipText.contains(ambientBrightnessTip)) {
            ...
            if (mIsLight) {
                ...
                mLightView!!.visibility = View.GONE
            }
        }
    }
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            ...
            R.id.select_image_tv -> PermissionsUtils.requsetRunPermission(this@TestScanActivity, object : RePermissionResultBack {
            ...
            R.id.flashLightLayout -> if (mIsLight) {
                mZXingView!!.closeFlashlight() // 打开闪光灯
                mIsLight = false
                mFlashLightIv!!.setImageResource(R.drawable.ic_close)
                mFlashLightTv!!.text = "打开闪光灯"
                ...
                mFlashLightTv!!.text = "关闭闪光灯"
            }
            R.id.albumLayout -> selectPic()
        }//mZXingView.getScanBoxView().setOnlyDecodeScanBoxArea(true); // 仅识别扫描框中的码
    }

    private fun selectPic() {
        // 进入相册 以下是例子：用不到的api可以不写
        PictureSelector.create(this@TestScanActivity)
...
*/
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == mSelectPicCode) {
            ...
            mZXingView!!.decodeQRCode(localMedia.path)
        }
    }
