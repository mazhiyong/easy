    ...
    public WindowInsets onApplyWindowInsets(WindowInsets insets) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (insets.getSystemWindowInsetBottom()>0){ //键盘开启
                ...
                if (!hasHomeKey) {
                    return super.onApplyWindowInsets(insets.replaceSystemWindowInsets(0, 0, 0, insets.getSystemWindowInsetBottom()-UtilTools.Companion.getNavigationBarHeight(mContenx)));
                }else {
